...
func GetConvertedValueForField(field *objects.Field, value interface{}) (interface{}, error) {
	if _, isBoolean := value.(bool); !isBoolean && field.Type == "boolean" {
		switch x := value.(type) {
			case bool: return x, nil
			case int: if x == 0 { return false, nil } else { return true, nil }
			case string: if strings.ToLower(x) == "false" { return false, nil } else { return true, nil }
			default: return nil, errors.New(fmt.Sprintf("%v is not a valid boolean value for boolean field %s", value, field.Name))
		}
	}
	return value, nil
}
